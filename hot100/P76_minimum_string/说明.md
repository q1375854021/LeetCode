...
public:
    ...
    bool compare(unordered_map<char, int> &source_map, unordered_map<char, int> &target_map,  string &t){
        for(auto m: t){
            if(source_map[m]<target_map[m]){
                return false;
            }
        }
        return true;
    ...
    int find_first(unordered_map<char, int> &target_map,  string &s, int begin_index){
...
但是找到了一个滑动窗口的特点，那就是右窗口连续滑动，for循环就可以，左窗口要使用while进行循环。

![image-20250610145319281](Image/image-20250610145319281.png)
